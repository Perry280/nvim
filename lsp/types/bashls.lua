...
---@field languageDialect? "auto" | "bash" | "posix" | "mksh" | "bats" = "auto" Language dialect to use when parsing
...
---@field includeAllWorkspaceSymbols? boolean = false Controls how symbols (e.g. variables and functions) are included and used for completion, documentation, and renaming. If false (recommended), then we only include symbols from sourced files (i.e. using non dynamic statements like 'source file.sh' or '. file.sh' or following ShellCheck directives). If true, then all symbols from the workspace are included.
